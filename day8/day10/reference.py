...
def play_game():
    ...
    def deal_card() -> int:
        """Return a random card(number) from deck"""
        cards = [11, 2, 3, 4, 5, 6, 7, 8, 9, 10, 10, 10, 10]
    ...
    def calculate_score(cards) -> int:
        ...
        if sum(cards) == 21 and len(cards) == 2:  # a hand with only 2 cards: ace + 10
    ...
    while not is_game_over:
        ...
        # game end condition: 'black jack' or 'continue' 
        if user_score == 0 or computer_score == 0 or user_score > 21: #black jack or 
            is_game_over = True
        else:
